...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		List<String> explored = new ArrayList<String>();   // to track the words that you explored from queue
		List<String> retList = new LinkedList<String>();   // words to return
		if (numSuggestions <=0 || word.isEmpty()) {
			return retList;
		}
		...
		while (!queue.isEmpty() && retList.size() < numSuggestions && explored.size() <= THRESHOLD) {
			String s = queue.remove();
			explored.add(s);
			List<String> neighbors = new ArrayList<String>();
			insertions(s, neighbors, false);
			substitution(s, neighbors, false);
			deletions(s, neighbors, false);
