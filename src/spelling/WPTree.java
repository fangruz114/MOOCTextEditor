...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		List<String> path = new LinkedList<String>(); // store word path, to be returned
		root = new WPTreeNode(word1, null); // set the root to be WPTreeNode containing word1
		
		LinkedList<WPTreeNode> queue = new LinkedList<WPTreeNode>(); // track the words to be explored
		queue.add(root); // add root to the queue
		HashSet<String> visited = new HashSet<String>();// track all the words to avoid exploring one word twice
		visited.add(word1); // add the initial word to visited
		
		List<String> curr = new ArrayList<String>(); // store words one step away from the word removed and explored from queue
		List<String> explored = new ArrayList<String>(); //track how many word already been explored
		
		while (!queue.isEmpty() && explored.size() <= THRESHOLD) {
			WPTreeNode remove = queue.remove();
			explored.add(remove.getWord());
			curr = nw.distanceOne(remove.getWord(), true);
			for (String s: curr) {
				if (!visited.contains(s)) {
					visited.add(s);
					WPTreeNode n = new WPTreeNode(s,remove);
					queue.add(n);
					if (s.equals(word2)) {
						path = n.buildPathToRoot();
						return path;
					}
				}
			}
